...
| **`orb_montecarlo.py`** | Block-bootstrap (block=10) Monte Carlo + **prop-eval race** simulator. For each account (25K/50K) simulates day-by-day until profit target (PASS) / EOD trailing-DD breach (BLOW) / 252 days (TIMEOUT). Models both "lock at start balance" and strict always-trailing DD. Produces the survival table in [`RESULTS.md`](RESULTS.md#monte-carlo--prop-survival). ⚠️ Its clock counts *trades* as days (~47% session density) — "77 days to pass" ≈ 244 calendar days. Superseded for planning by `_scaling_mc.py`. |
| **`_scaling_mc.py`** | **Session-clock lifecycle Monte Carlo** (2026-07-01): true RTH-session calendar (zero-P&L no-trade days, cached in `_sessions_rth.csv`), eval race + 50% consistency rule, eval **campaigns** (blow → $95 reset → retry), funded stage with **buffer-based contract scaling** (1 micro per $step of headroom, capped) + monthly payout sweeps, and the full eval→funded→blow→new-eval **lifecycle**. Both DD-lock models. Source of the "~$3k/yr → ~$6–9k/yr per slot" table in CONTEXT.md §3e. |
